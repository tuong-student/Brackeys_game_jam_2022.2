    ...
    public class QuickCheckWindow : EditorWindow
    {
        ...
        void Refresh()
        {
            ...
            foreach (Type type in asm.GetTypes())
            {
                ...
                {
                    var attrc = method.GetCustomAttribute<QuickCheckCollectionAttribute>();

                    if (attrc != null)
                    {
                        Tuple<string, Action<int>, int>[] tuples = method.Invoke(null, null) as Tuple<string, Action<int>, int>[];

                        if (tuples != null)
                        {
                            foreach (var tuple in tuples)
                            {
                                Action<int> action = tuple.Item2;
                                MethodInfo targetMethod = methods.Find(m => m.Name == action.Method.Name);

                                items.Add(new CheckTreeItem(labelNames, targetMethod.Name, targetMethod.Name + " (" + tuple.Item1 + ")", () =>
                                 {
                                     OnButtonDown(type, targetMethod, new int[1] { tuple.Item3 });
                                 }));
                            }
                        }
                    }
        ...
        void OnButtonDown(Type type, MethodInfo method, int[] intArgs = null, string[] stringArgs = null)
        {
            ...
            if (!EditorApplication.isPlaying)
            {
                ...
                if (currentScene.path != "" && !currentScene.path.Contains("QuickTestScene"))
                {
        ...
        static void OnPlaymodeStateChanged(PlayModeStateChange state)
        {
            if (state == PlayModeStateChange.EnteredEditMode)
            {
                ...
                else if (EditorSceneManager.GetActiveScene().name.Contains("QuickTestScene"))
                {
